    ...
    try {
        ...
        if (pn == 1) {
            ...
            for (var i = 0; i < clst.length; i++) {
                ...
                } else if (/dsxys|vipmv|haokanju|5scrn|newfii|netflix|4kcz|1090ys|duanju/.test(spl)) {
...
function clsrule() {
    ...
    for (var i = 0; i < list.length; i++) {
        //图片
        try {
            if (/auete|haozhansou|wwgz|bdys|yjys/.test(MY_URL)) {
        ...
        //标题
        if (/czzzu|czys|cz01|czzy|magedn|subaibai|nfyingshi|renrenmi|bdys|yjys/.test(MY_URL)) {
...
function hiksearch() {
    var urlph = $.toString(() => {
        ...
        } else if (/5scrn|3443|cokemv|haokanju|4kcz|newfii|netflix|ak1080|1090ys|zxzj|gfvod|lekkan|dadou|duanju|jukankan/.test(url)) {
    ...
    if (ssmd == '0' || MY_TYPE == "home") {
        for (var i = 0; i < json.data.length; i++) {
            ...
            for (var j = 0; j < list.length; j++) {
                var url = list[j].url;
                eval(urlph);
                if (/zhenbuka|ikandy|3443|cokemv|tvyb|saohuo|shdy3|shdy2|kanju77|tegouys|tkznp|ak1080|lezhutv|nkvod|7xdy/.test(url)) {
...
function hikseaerji() {
    ...
    if (html.search(/请输入验证码|验证后查看搜索结果|首次搜索需要输入验证码|访问此数据需要输入验|正确的验证码继续访问|需要先输入验证码/) != -1 && !/wwgz/.test(spl)) {
        ...
        //验证码输入处理
            ...
            } else if (spl.search(/bdys|yjys/) != -1) {
        ...
        if (html.indexOf('id="torrent-list') != -1) {
            ...
            for (var i = 0; i < dnli.length; i++) {
                d.push({
                    title: pdfh(dnli[i], "a&&Text"),
                    col_type: 'text_center_1',
                    url: pd(dnli[i], "a&&href")+";{Referer@https://www.yjys05.com/}"
                });
            }
        }
...
function omlazy() {
    ...
    try {
        ...
        } else if (/wwgz|5scrn|aiyy|shigys|v\.t-ui|dadou|kanyk|oftens|wwys|nkvod|7xdy/.test(myurl)) {
            //直接网页嗅探
            return x5rule(srcurl, srcurl);
        } else if(/bdys|yjys|newfii/.test(myurl)){
            return 'video://'+srcurl;
